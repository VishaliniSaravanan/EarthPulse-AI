...
# company -> list of chunk dicts
COMPANY_CHUNKS: dict[str, list[dict]] = {}


def build_graph_augmented_rag_graph(chunks: list[dict], company: str):
    """Build a lightweight graph over semantic chunks for a company."""
    G = nx.Graph()
    ...
    for _, indices in section_indices.items():
        for j in range(len(indices) - 1):
            G.add_edge(indices[j], indices[j + 1], weight=1.0, relation="sequential")

    # Cross-section edges for related keyword sections (heuristic)
    RELATED = {
...
def graph_augmented_rag_query(query: str, company: str = None, k: int = 4) -> list[dict]:
    """Retrieve chunks via vector search, then expand using the company graph."""
...
def get_graph_augmented_rag_export(company: str) -> dict:
    """Return graph nodes/edges in the format the frontend ForceGraph expects."""
