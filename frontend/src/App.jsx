...
import {
  ...
  DollarSign, Zap, Network, Wifi, WifiOff, ChevronRight, FileSearch
} from 'lucide-react'
...
const NAV = [
  { key:'upload',       label:'Upload Report',  icon:Upload,      group:'START'        },
  { key:'pdfview',      label:'PDF Analyzer',   icon:FileSearch,  group:'START'        },
  { key:'metrics',      label:'ESG Metrics',    icon:BarChart2,   group:'ANALYTICS'    },
  { key:'query',        label:'Query Engine',   icon:Search,      group:'ANALYTICS'    },
  { key:'greenwashing', label:'Greenwashing',   icon:ShieldAlert, group:'ANALYTICS'    },
  { key:'supply',       label:'Supply Chain',   icon:Link2,       group:'ANALYTICS'    },
  { key:'climate',      label:'Climate Risk',   icon:Globe2,      group:'INTELLIGENCE' },
  { key:'financing',    label:'Financing',      icon:DollarSign,  group:'INTELLIGENCE' },
  { key:'scenario',     label:'Scenarios',      icon:Zap,         group:'INTELLIGENCE' },
  { key:'graphs',       label:'Graphs',         icon:Network,     group:'INTELLIGENCE' },
]
...
export default function App() {
  ...
  // Keep a reference to the uploaded File object so PDFAnalysisPage can render it
  ...
  useEffect(() => {
    const onPopState = () => setPage('landing')
    window.addEventListener('popstate', onPopState)
    return () => window.removeEventListener('popstate', onPopState)
  }, [])

  const handleAnalyzed = (result, file) => {
    pdfFileRef.current = file   // stash the File so PDFAnalysisPage can load it
  ...
  if (page === 'landing') {
    return <LandingPage onStart={goToUpload} />
  }
  ...
  return (
    <div style={{ display:'flex', minHeight:'100vh' }}>
      ...
      <div className="sidebar" style={{ display:'flex', flexDirection:'column' }}>
        ...
        <div style={{ padding:'16px 16px 12px', borderBottom:'1px solid var(--border)' }}>
          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
            ...
            <div>
              ...
              <div style={{ fontSize:10, color:'var(--sidebar-text3)', fontFamily:'JetBrains Mono' }}>v2.0 Platform</div>
            </div>
          </div>
        </div>
        ...
        {company && (
          <div style={{ padding:'10px 12px', borderBottom:'1px solid var(--border)', background:'rgba(46,125,50,0.12)' }}>
            <div style={{ fontSize:10, color:'var(--sidebar-text2)', fontWeight:600, textTransform:'uppercase', letterSpacing:'0.05em', marginBottom:2 }}>Active Report</div>
        ...
        <nav style={{ flex:1, padding:'8px', overflowY:'auto' }}>
          {groups.map(group => (
            <div key={group} style={{ marginBottom:8 }}>
              <div style={{ fontSize:9, fontWeight:700, color:'var(--sidebar-text3)', padding:'8px 8px 4px', letterSpacing:'0.1em', fontFamily:'JetBrains Mono' }}>
                {group}
              </div>
              {NAV.filter(n => n.group === group).map(({ key, label, icon:Icon }) => {
                const isActive = page === key
                const isLocked = key !== 'upload' && !company
                return (
                  <button
                    key={key}
                  ...
                  >
                    ...
                    {isLocked && <span style={{ fontSize:9, color:'var(--sidebar-text3)', fontFamily:'JetBrains Mono' }}>LOCKED</span>}
                    {key === 'pdfview' && company && !isActive && (
                      <span style={{ fontSize:8, background:'rgba(46,125,50,0.2)', color:'var(--sidebar-text)', padding:'1px 5px', borderRadius:99, fontFamily:'JetBrains Mono' }}>NEW</span>
                    )}
                  </button>
                )
              })}
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', minWidth:0, background:'var(--main-bg)' }}>
        ...
        {/* Page content — PDF Analyzer is edge-to-edge */}
        <div style={{
          flex:1,
